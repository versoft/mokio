...
function sortablephotos(){
  $('.sortable').sortable({
    ...
    stop: function(event, ui) {
      ...
      $.post( '/' + Mokio.engine_root() + '/'+data_file_model+'/sort/', { ids_order: $idsInOrder } ); // ajax to sort photos
      prettyphoto();
    }
  }).disableSelection();
}
...
function photo_options_fields() {
  ...
  $(".photo-options-fields li.photo_thumb").click(function(event) {
    ...
    setTimeout(function() {                                                                    //
      ...
      $.get('/' + Mokio.engine_root() +'/'+default_data_file+'/' + $id + '/get_thumb', function(data) { });
    }, 500);
  });
