...
$(document).on('ready page:load', function(){
  ...
  var mokioSelect2 = ".mokio-select2";

  $(document).find(mokioSelect2).each(function() {
    var dataAjax = $(this).attr('data-ajax-collection');
  ...
  $("#histories").on("shown",function() {
    $('#histories .input_content').each(function(index) {
      var content = $(this);
      var contentHeight = content.innerHeight();
      var contentMaxHeight = content.css('max-height');

      if(contentMaxHeight.length > 0) {
        contentMaxHeight = parseInt(contentMaxHeight, 10);
      }

      var show_more_btn = content.closest('td').find('.show_more');

      if(contentHeight  >= contentMaxHeight ) {
        show_more_btn.show();
        show_more_btn.click(function(){
          var row  = 	content.closest('tr');
          row.find('.input_content').animate(100).css('max-height','100%');
          row.find('.show_more').hide();
        });
      }
    });
  });
