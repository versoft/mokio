    ...
    module Models
      ...
      module AvailableModule
        ...
        included do
          ...
          scope :static_module_without_always_displayed_and_active_for_lang, -> (position_id,lang_id) {joins("join mokio_static_modules on mokio_available_modules.static_module_id = mokio_static_modules.id").where('(mokio_static_modules.lang_id = ? AND mokio_static_modules.active = true AND mokio_available_modules.module_position_id = ? AND mokio_static_modules.and always_displayed = false) ', lang_id,position_id).select("mokio_static_modules.*,mokio_available_modules.*")}
          scope :static_module_with_always_displayed_and_active_for_lang, -> (position_id,lang_id) {joins("join mokio_static_modules on mokio_available_modules.static_module_id = mokio_static_modules.id").where('(mokio_static_modules.lang_id = ? AND mokio_static_modules.active = true AND mokio_available_modules.module_position_id = ? AND mokio_static_modules.always_displayed = true) ', lang_id,position_id).select("mokio_static_modules.*,mokio_available_modules.*")}
