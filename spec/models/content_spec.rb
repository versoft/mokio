...
module Mokio
  describe Content do
    ...
    it "has a valid factory" do
      FactoryBot.create(:content).should be_valid
    end

    it 'is invalid without title' do
      FactoryBot.build(:content_without_title).should_not be_valid
    end
    ...
    it "should have many data_files" do
      ...
      expect { FactoryBot.create(:content).data_files }.to_not raise_error
    end
    ...
    describe "simple content" do
      it 'has content_type = nil' do
        FactoryBot.build(:content).type.should be_nil
      end

      context "with main pic" do
        it 'is valid' do
          FactoryBot.create(:content_with_main_pic).should be_valid
        end

        it 'peding saves association with data_file' do
          content = FactoryBot.create(:content_with_main_pic)
          content.save!
    ...
    describe 'functions' do
      ...
      context 'displayed' do
        it 'is true for nil display_from and display_to' do
          content =  FactoryBot.create(:content_display_to_from_nil)
          expect(content.displayed?).to be_truthy
        end

        it 'is true for display_from in the past and nil display_to' do
          content =  FactoryBot.create(:content_display_to_nil_displayed)
          expect(content.displayed?).to be_truthy
        end

        it 'is true for nil display_from and display_to in the future' do
          content =  FactoryBot.create(:content_display_from_nil_displayed)
          expect(content.displayed?).to be_truthy
        end

        it 'is false for display_from and display_to out of range' do
          content =  FactoryBot.create(:content_not_displayed)
          expect(content.displayed?).to be_falsey
        end

        it 'is false for display_from in the future and nil display_to' do
          content =  FactoryBot.create(:content_display_to_nil_not_displayed)
          expect(content.displayed?).to be_falsey
        end

        it 'is false for nil display_from and display_to in the past' do
          content =  FactoryBot.create(:content_display_from_nil_not_displayed)
          expect(content.displayed?).to be_falsey
        end
      end
    end
